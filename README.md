...
- GitHub Copilot Chat：优先通过 Copilot 的“导出聊天日志”JSON 文件统计；未配置导出路径时会尝试扫描 Copilot 的 `globalStorage`（best-effort，可能不完整）
...
- `tokenConsumptionStatistics.copilot.enableHeuristicScan`: 未配置导出路径时，启用 Copilot `globalStorage` 启发式扫描
...
- Kilo Code 会尝试在 `globalStorage` 下匹配多个可能的扩展目录名（不同版本可能变动）。
- Copilot 的启发式扫描依赖其落盘日志结构，可能统计不完整；更稳定的方式仍然是导出 JSON。
